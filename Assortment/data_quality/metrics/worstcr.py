...
def check_lp(real_set, gen_set, K_list, n, eps=1e-1):
    ...
    A_ub.append(np.array(x_real, dtype=float))
    b_ub.append(1.0)

    A_ub.append(np.array(x_gen, dtype=float))
    b_ub.append(1.0)

    for K in K_list:
        xK = to_vec(K, n)
        A_ub.append(-np.array(xK, dtype=float))
        b_ub.append(-(1.0 + eps))

    A_ub = np.vstack(A_ub) if len(A_ub) > 0 else None
    b_ub = np.array(b_ub, dtype=float) if len(b_ub) > 0 else None

    # Objective doesn't matter (feasibility). Use zeros.
    c = np.zeros(n, dtype=float)

    res = linprog(c=c, A_ub=A_ub, b_ub=b_ub, bounds=bounds, method="highs")

    if res.status != 0:  # not optimal => infeasible or numerical issue
        return False, None
    return True, res.x
